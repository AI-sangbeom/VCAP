...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.prompt = nn.Parameter(torch.randn(1, 197, prompt_dim))
        self.prompt_proj = nn.Linear(prompt_dim, dim)
        
        
    def forward(self, x):
        x = x + self.drop_path(self.attn(self.norm1(x)))
        x = x + self.drop_path(self.mlp(self.norm2(x))) + self.prompt_proj(self.prompt)
        return x


class VisionTransformerwithPrompt(nn.Module):
    """ Vision Transformer
